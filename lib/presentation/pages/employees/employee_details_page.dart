...
import '../../cubits/weekly_allowance/weekly_allowance_cubit.dart';
import '../../cubits/weekly_allowance/weekly_allowance_state.dart';
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider<WeeklyAllowanceCubit>(
          create: (_) => sl<WeeklyAllowanceCubit>()
            ..loadWeeklyAllowance(employeeId),
        ),
      ],
...
class _EmployeeDetailsViewState extends State<_EmployeeDetailsView> with SingleTickerProviderStateMixin {
  ...
  // ── Weekly Work Allowance Dialogs ──────────────────────────────────────────
  void _showAddAllowanceDialog(
    BuildContext context,
    String employeeName,
    WeeklyAllowanceLoaded state,
  ) {
    final l10n = AppLocalizations.of(context)!;
    final amountController = TextEditingController();
    final noteController = TextEditingController();
    ExpenseCurrency selectedCurrency = state.selectedCurrency;
    DateTime selectedDate = DateTime.now();
    final formKey = GlobalKey<FormState>();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.account_balance_wallet_outlined, color: Color(0xFF0984E3)),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(l10n.addAllowance, style: AppTextStyles.heading3),
                              Text(
                          ...
                          children: [
                            Row(
                              children: [
                                const Icon(Icons.calendar_today_outlined, size: 18, color: AppColors.textSecondary),
                                const SizedBox(width: 10),
                                Text(l10n.allowanceDateLabel, style: AppTextStyles.bodyMedium),
                              ],
                            ),
                            Text(
                              DateFormat('yyyy/MM/dd').format(selectedDate),
                    ...
                    TextFormField(
                      ...
                      maxLines: 2,
                      decoration: InputDecoration(
                        labelText: l10n.allowanceReasonLabel,
                        hintText: l10n.allowanceReasonHint,
                        prefixIcon: const Padding(
                          padding: EdgeInsets.only(bottom: 30),
                          child: Icon(Icons.notes_outlined),
                        ),
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              if (formKey.currentState!.validate()) {
                                final amount = double.parse(amountController.text.trim());
                                final note = noteController.text.trim();
                                context.read<WeeklyAllowanceCubit>().addAllowance(
                                      userId: widget.employeeId,
                                      amount: amount,
                                      currency: selectedCurrency,
                                      transactionDate: selectedDate,
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(l10n.allowanceAddedSuccess),
                                    backgroundColor: AppColors.success,
                                  ),
                                );
                              }
                            },
                            child: Text(l10n.addAllowance),
                          ),
                        ),
                      ],
                    ),
  ...
  void _showEditAllowanceDialog(
    ...
    ExpenseCurrency selectedCurrency = tx.currency;
    ...
    showDialog(
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: Row(
            children: [
              const Icon(Icons.edit_note, color: Color(0xFF0984E3)),
              const SizedBox(width: 8),
              Flexible(child: Text(l10n.editAllowance, style: AppTextStyles.subtitle1.copyWith(fontWeight: FontWeight.bold))),
            ],
          ),
          content: Form(
            key: formKey,
            ...
            ElevatedButton(
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  final amount = double.parse(amountController.text.trim());
                  final note = noteController.text.trim();
                  context.read<WeeklyAllowanceCubit>().updateAllowance(
                        id: tx.id,
                        userId: widget.employeeId,
                        amount: amount,
                        currency: selectedCurrency,
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(l10n.allowanceUpdatedSuccess),
                      backgroundColor: AppColors.success,
                    ),
                  );
                }
  ...
  void _confirmDeleteAllowance(BuildContext context, WeeklyAllowanceTransaction tx) {
    final l10n = AppLocalizations.of(context)!;
    final cubit = context.read<WeeklyAllowanceCubit>();

    showDialog(
      context: context,
      builder: (dialogCtx) => AlertDialog(
        title: Text(l10n.deleteAllowance),
        content: Text(l10n.deleteAllowanceConfirm),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogCtx),
            child: Text(l10n.cancel),
          ...
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(dialogCtx);
              cubit.deleteAllowance(tx.id, widget.employeeId);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(l10n.allowanceDeletedSuccess),
                  backgroundColor: AppColors.success,
                ),
              );
            },
            child: Text(l10n.deleteAllowance),
          ),
        ],
      ),
    );
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ...
              context.read<WeeklyAllowanceCubit>().loadWeeklyAllowance(widget.employeeId);
            },
          ),
        ],
      ),
              ...
              return NestedScrollView(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // 2. Weekly Work Budget Card (ميزانية العمل الأسبوعية)
                            BlocBuilder<WeeklyAllowanceCubit, WeeklyAllowanceState>(
                              builder: (context, weeklyState) {
                                final loaded = weeklyState is WeeklyAllowanceLoaded ? weeklyState : null;
                                final weekRange = loaded?.weekRange ?? DateTimeUtils.getThisWeekRange();
                                final selectedCurrency = loaded?.selectedCurrency ?? ExpenseCurrency.egp;
                                final received = loaded?.activeReceived ?? 0.0;
                                final spent = loaded?.activeSpent ?? 0.0;
                                final remaining = loaded?.activeRemaining ?? 0.0;

                                return Container(
                                  padding: const EdgeInsets.all(16),
                                  decoration: BoxDecoration(
                                    gradient: const LinearGradient(
                                      colors: [Color(0xFF0984E3), Color(0xFF0070BA)],
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          if (loaded != null)
                                            ElevatedButton.icon(
                                              onPressed: () => _showAddAllowanceDialog(context, profile.name, loaded),
                                      ...
                                      // Week Period Controls (Previous Week, This Week, Next Week)
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Text(
                                            weekRange.format(isArabic ? 'ar' : 'en'),
                                          ...
                                          Row(
                                            children: [
                                              IconButton(
                                                ...
                                                onPressed: () {
                                                  final prevRange = DateTimeUtils.getWeekRange(weekRange.start.subtract(const Duration(days: 7)));
                                                  context.read<WeeklyAllowanceCubit>().selectPeriod(
                                                        WeeklyPeriodSelection.custom,
                                                        customRange: prevRange,
                                                      );
                                                },
                                              ),
                                              const SizedBox(width: 8),
                                              InkWell(
                                                onTap: () {
                                                  context.read<WeeklyAllowanceCubit>().selectPeriod(WeeklyPeriodSelection.thisWeek);
                                                },
                                                child: Container(
                                              ...
                                              IconButton(
                                                ...
                                                onPressed: () {
                                                  final nextRange = DateTimeUtils.getWeekRange(weekRange.start.add(const Duration(days: 7)));
                                                  context.read<WeeklyAllowanceCubit>().selectPeriod(
                                                        WeeklyPeriodSelection.custom,
                                      ...
                                      // Currency Switch & 3 Metric Tiles
                                      Row(
                                        children: [
                                          // Received
                                          Expanded(
                                            child: Container(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    l10n.receivedThisWeek.split(' ').first,
                                                    style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w600),
                                                  ),
                                                  const SizedBox(height: 4),
                                                  Text(
                                                    _formatAmount(received, selectedCurrency, isArabic),
                                          ...
                                          Expanded(
                                            child: Container(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    l10n.spentThisWeek.split(' ').first,
                                                    style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w600),
                                                  ),
                                                  const SizedBox(height: 4),
                                                  Text(
                                                    _formatAmount(spent, selectedCurrency, isArabic),
                                                    style: const TextStyle(color: Color(0xFFFFD2D2), fontSize: 13, fontWeight: FontWeight.bold),
                                                  ),
                                                ],
                                              ),
                                            ),
                                          ...
                                          const SizedBox(width: 8),
                                          // Remaining (negative if spent > received)
                                          Expanded(
                                            child: Container(
                                              padding: const EdgeInsets.all(10),
                                              decoration: BoxDecoration(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Text(
                                                    l10n.remainingThisWeek.split(' ').first,
                                                    style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w600),
                                                  ),
                                                  const SizedBox(height: 4),
                                                  Text(
                                                    _formatAmount(remaining, selectedCurrency, isArabic),
                                                    style: TextStyle(
                                                      color: remaining >= 0 ? const Color(0xFF55EFC4) : const Color(0xFFFF7675),
                                                      fontSize: 13,
                                                      fontWeight: FontWeight.bold,
                                                    ),
                                                  ),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          InkWell(
                                            onTap: () => context.read<WeeklyAllowanceCubit>().changeCurrency(ExpenseCurrency.egp),
                                            child: Container(
                                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                              decoration: BoxDecoration(
                                                color: selectedCurrency == ExpenseCurrency.egp
                                                    ? Colors.white
                                          ...
                                          const SizedBox(width: 6),
                                          InkWell(
                                            onTap: () => context.read<WeeklyAllowanceCubit>().changeCurrency(ExpenseCurrency.usd),
                                            child: Container(
                                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                              decoration: BoxDecoration(
                                                color: selectedCurrency == ExpenseCurrency.usd
                                                    ? Colors.white
                                                    : Colors.white.withValues(alpha: 0.2),
                                                borderRadius: BorderRadius.circular(6),
                                              ),
                            ...
                            Container(
                              ...
                              child: TabBar(
                                ...
                                tabs: [
                                  ...
                                  Tab(text: l10n.weeklyAllowanceTransactions),
                ...
                body: TabBarView(
                  ...
                  children: [
                    ...
                    // Tab 2: Weekly Allowance Transactions (حركات العهد)
                    _buildWeeklyAllowanceTab(context, profile, l10n, isArabic, dateFormat),

                    // Tab 3: Full Financial Transaction History (Credits + Expenses)
                    _buildFinancialHistoryTab(context, l10n, isArabic, dateFormat),

                    // Tab 4: Salary Advances (Advances history + Add/Edit/Delete)
  ...
  Widget _buildWeeklyAllowanceTab(
    BuildContext context,
    Profile profile,
    ...
    return BlocBuilder<WeeklyAllowanceCubit, WeeklyAllowanceState>(
      builder: (context, weeklyState) {
        if (weeklyState is WeeklyAllowanceLoading) {
          return const Center(child: CircularProgressIndicator());
        }
        ...
        return ListView(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  '${l10n.weeklyAllowanceTransactions} (${transactions.length})',
                  style: AppTextStyles.heading3,
                ),
                if (loaded != null)
                  ElevatedButton.icon(
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 12),
                        Text(
                          l10n.noAllowanceTransactionsYet,
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      backgroundColor: const Color(0xFF0984E3).withValues(alpha: 0.15),
                      child: const Icon(
                    ...
                    title: Text(
                      '+${_formatAmount(tx.amount, tx.currency, isArabic)}',
                      style: AppTextStyles.subtitle2.copyWith(
                        fontWeight: FontWeight.bold,
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text(
                          '${l10n.allowanceDateLabel}: ${dateFormat.format(tx.transactionDate)}${tx.note != null && tx.note!.isNotEmpty ? " • ${tx.note}" : ""}',
                          style: AppTextStyles.caption,
                        ),
                        if (tx.creatorName != null && tx.creatorName!.isNotEmpty)
                          Text(
                            '${l10n.advanceCreatedBy}: ${tx.creatorName}',
                    ...
                    trailing: PopupMenuButton<String>(
                      ...
                      onSelected: (action) {
                        if (action == 'edit' && loaded != null) {
                          _showEditAllowanceDialog(context, tx, loaded);
                        } else if (action == 'delete') {
                          _confirmDeleteAllowance(context, tx);
                        }
                      },
                      itemBuilder: (ctx) => [
                        PopupMenuItem(
                        ...
                        PopupMenuItem(
                          ...
                          child: Row(
                            children: [
                              ...
                              const SizedBox(width: 8),
                              Text(l10n.deleteAllowance, style: const TextStyle(color: AppColors.error)),
                            ],
                          ),
                        ),
                      ],
