-- ==============================================================================
-- Spendly / Egypt Edu Gate - Migration 017: Fix Admin User Auth & Realtime Sync
-- Purpose:
-- 1. Fix admin_create_user RPC to ensure full GoTrue / Supabase Auth compatibility:
--    - provider_id in auth.identities is correctly set to user_id::text
--    - confirmation_token is set to empty string '' for pre-confirmed emails
--    - identity_data and raw_user_meta_data contain standard sub/email keys
-- 2. Repair existing auth.identities records created with legacy provider_id
-- 3. Enable Supabase Realtime publication on public.profiles
-- ==============================================================================

-- ── 1. Update public.admin_create_user RPC with GoTrue Contract ───────────────
...
BEGIN
    ...
    -- 5. Insert directly into auth.users with verified status and standard metadata
    ...
    -- 6. Insert identity into auth.identities with provider_id = user_id::text
    BEGIN
...
-- ── 2. Backfill & Repair Existing Auth Records ────────────────────────────────
-- Fix any identities where provider_id was saved as email instead of user_id UUID
UPDATE auth.identities
SET provider_id = user_id::text,
    identity_data = jsonb_build_object(
        'sub', user_id::text,
        'email', lower(COALESCE(identity_data->>'email', provider_id)),
        'email_verified', true,
        'phone_verified', false
    )
WHERE provider = 'email' AND provider_id <> user_id::text;

-- Ensure all auth.users have an auth.identities entry
INSERT INTO auth.identities (
    id,
    user_id,
    identity_data,
...
-- Sanitize and Repair All Existing auth.users Records
UPDATE auth.users
SET 
...
-- ── 3. Enable Supabase Realtime for Profiles ──────────────────────────────────
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    -- Add admin_notifications only if table exists in this environment
    IF EXISTS (
...
-- ── 4. Grant Permissions & Reload Schema Cache ───────────────────────────────
